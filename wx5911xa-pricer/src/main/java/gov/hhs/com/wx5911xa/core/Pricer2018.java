...
import gov.hhs.com.wx5911xa.viewmodel.*;
...
public class Pricer2018 implements iPricer {
  private WageIndexMap wageIndexMap;
  private CaseMixGroupCodeTable cmgTable;

  // TODO: Rename these to something informative
  private static final String PRICER_VERSION_2018 = "V18.0";
  ...
  private static final BigDecimal NATIONAL_LABOR_PERCENTAGE = BigDecimal.valueOf(0.70700);
  private static final BigDecimal NATIONAL_NON_LABOR_PERCENTAGE = BigDecimal.valueOf(0.29300);
  private static final BigDecimal NATIONAL_THRESHOLD_ADJUSTMENT = BigDecimal.valueOf(8679.00);
  private static final BigDecimal BUDGET_NEUTRAL_CONVERSION_AMOUNT = BigDecimal.valueOf(15838.00);
  private static final BigDecimal BUDGET_NEUTRAL_CONVERSION_AMOUNT2 = BigDecimal.valueOf(15524.00);
  ...
  // TODO: These should all probably be enums...
  ...
  private static final Set<String> ORTHOPEDIC_CMG_PREFIX =
      new HashSet<String>(Arrays.asList("07", "08", "09"));
  // This is the list of rural to urban providers; list not used for rural
  // adjustment in 2018
  /*
   * private static final Set<String> VALID_PROVIDERS = new HashSet<String>(
  ...
  public Pricer2018(WageIndexMap wageIndexMap, CaseMixGroupCodeTable cmgTable) {
  ...
  protected CoreBasedStatisticalArea getCbsa(String dischargeDate, ProviderRecord provider) {
    ...
    if (cbsa.getSpecialPaymentIndicator().equals(SPECIAL_PAY_INDICATOR)) {
      return new CoreBasedStatisticalArea(cbsa.getGeolocation(), "00000000", cbsa.getWageIndex());
    }
  ...
  protected void checkRuralAdj(
     ...
     * ppsRecord.setRuralAdjustment(RURAL_ADJUSTMENT); } else
     */
    if (cbsaInt < 100) {
  ...
  protected PricerResult calcPayment(
     ...
     * ppsRecord.setFederalPaymentAmount((ppsRecord.getFederalPaymentAmount() -
     * ppsRecord.getFederalPenaltyAmount())); // ** RULE: ALL CLAIMS WITH PENALTIES
     * HAVE THEIR PPS LOW INCOME // ** PATIENT PENALTY AMOUNT COMPUTED AS THE (PPS
     * LOW INCOME PATIENT // ** PAY AMOUNT MINUS THE PPS LOW INCOME PATIENT PENALTY
     * AMOUNT) // ** ROUNDED. ppsRecord.setLowIncomePaymentPenaltyAmount(ppsRecord.
     * getLowIncomePaymentAmount() * TWENTY_FIVE_PERCENT); // ** RULE: ALL CLAIMS
     * WITH PENALTIES HAVE THEIR PPS OUTLIER PENALTY // ** AMOUNT COMPUTED AS THE
     * (PPS OUTLIER PAY AMOUNT TIMES 25%) // ** ROUNDED. IN OTHER WORDS, THEY PAY A
     * 25% OUTLIER PENALTY
     * ppsRecord.setLowIncomePaymentAmount((ppsRecord.getLowIncomePaymentAmount() -
     ...
     * AMOUNT ppsRecord.setOutlierPaymentAmount((ppsRecord.getOutlierPaymentAmount()
     * - ppsRecord.getOutlierPenaltyAmount())); // ** RULE: ALL CLAIMS WITH
     * PENALTIES HAVE THEIR PPS TEACH PENALTY // ** AMOUNT COMPUTED AS THE (PPS
     * TEACH PAY AMOUNT MINUS THE PPS THE // ** PPS TEACH PENALTY AMOUNT) ROUNDED.
     ...
     * ppsRecord.getTeachPenaltyAmount()));
     * ppsRecord.setTotalPenaltyAmount((ppsRecord.getFederalPenaltyAmount() +
