...
public class Pricer2017 implements iPricer {
  private WageIndexMap wageIndexMap;
  private CaseMixGroupCodeTable cmgTable;

  private static final String PRICER_VERSION_2017 = "V17.0";
  ...
  private static final BigDecimal TWENTY_FIVE_PERCENT = BigDecimal.valueOf(0.25);
  ...
  private static final BigDecimal NATIONAL_LABOR_PERCENTAGE = BigDecimal.valueOf(0.70900);
  private static final BigDecimal NATIONAL_NON_LABOR_PERCENTAGE = BigDecimal.valueOf(0.29100);
  private static final BigDecimal NATIONAL_THRESHOLD_ADJUSTMENT = BigDecimal.valueOf(7984.00);
  private static final BigDecimal BUDGET_NEUTRAL_CONVERSION_AMOUNT = BigDecimal.valueOf(15708.00);
  private static final BigDecimal BUDGET_NEUTRAL_CONVERSION_AMOUNT2 = BigDecimal.valueOf(15399.00);
  private static final BigDecimal RURAL_ADJUSTMENT = BigDecimal.valueOf(1.0497);
  ...
  private static final Set<String> ORTHOPEDIC_CMG_PREFIX =
      new HashSet<>(Arrays.asList("07", "08", "09"));
  private static final Set<String> VALID_PROVIDERS =
      new HashSet<>(
          Arrays.asList(
              "013027", "08T009", "10T249", "14T011", "193044", "193079", "193096", "19T003",
              "19T144", "19T304", "28T023", "33T157", "34T015", "34T131", "393047", "39T151",
              "42T036", "42T067", "49T018", "50T002"));

  public Pricer2017(WageIndexMap wageIndexMap, CaseMixGroupCodeTable cmgTable) {
  ...
  protected void checkRuralAdj(
    ...
    if (VALID_PROVIDERS.contains(bill.getProviderNumber())) {
      ppsRecord.setRuralAdjustment(RURAL_ADJUSTMENT);
    } else if (cbsaInt < 100) {
  ...
  protected PricerResult calcPayment(
    ...
    if (bill.hasPenalties()) {
      ...
      // ** PAY AMOUNT
      ppsRecord.setFederalPenaltyAmount(
          ppsRecord.getFederalPaymentAmount().multiply(TWENTY_FIVE_PERCENT));
      ...
      ppsRecord.setFederalPaymentAmount(
          (ppsRecord.getFederalPaymentAmount().subtract(ppsRecord.getFederalPenaltyAmount())));
      ...
      ppsRecord.setLowIncomePaymentPenaltyAmount(
          ppsRecord.getLowIncomePaymentAmount().multiply(TWENTY_FIVE_PERCENT));
      ...
      ppsRecord.setLowIncomePaymentAmount(
          (ppsRecord
              .getLowIncomePaymentAmount()
              .subtract(ppsRecord.getLowIncomePaymentPenaltyAmount())));
      // ** RULE: ALL CLAIMS WITH PENALTIES HAVE THEIR PPS OUTLIER PENALTY
      // ** AMOUNT COMPUTED AS THE (PPS OUTLIER PAY AMOUNT MINUS THE PPS
      // ** OUTLIER PENALTY AMOUNT) ROUNDED.
      ppsRecord.setOutlierPenaltyAmount(
          ppsRecord.getOutlierPaymentAmount().multiply(TWENTY_FIVE_PERCENT));
      ...
      // ** PAID AMOUNT
      ppsRecord.setOutlierPaymentAmount(
          (ppsRecord.getOutlierPaymentAmount().subtract(ppsRecord.getOutlierPenaltyAmount())));
      // ** RULE: ALL CLAIMS WITH PENALTIES HAVE THEIR PPS TEACH PENALTY
      // ** AMOUNT COMPUTED AS THE (PPS TEACH PAY AMOUNT MINUS THE PPS THE
      // ** PPS TEACH PENALTY AMOUNT) ROUNDED.
      ppsRecord.setTeachPenaltyAmount(
          ppsRecord.getTeachPaymentAmount().multiply(TWENTY_FIVE_PERCENT));
      ...
      ppsRecord.setTeachPaymentAmount(
          (ppsRecord.getTeachPaymentAmount().subtract(ppsRecord.getTeachPenaltyAmount())));
      ppsRecord.setTotalPenaltyAmount(
          (ppsRecord
              .getFederalPenaltyAmount()
              .add(ppsRecord.getLowIncomePaymentPenaltyAmount())
              .add(ppsRecord.getOutlierPenaltyAmount())
              .add(ppsRecord.getTeachPenaltyAmount())));
    }
